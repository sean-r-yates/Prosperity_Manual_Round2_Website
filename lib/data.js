...
function isValidParticipantAllocation(record) {
  return Boolean(
    record &&
    isFiniteNumber(record.researchPct) &&
    isFiniteNumber(record.scalePct) &&
    isFiniteNumber(record.speedPct)
  );
}
...
function normalizeAttemptRecord(record) {
  ...
  return {
    ...record,
    researchPct: Number(record.researchPct),
    scalePct: Number(record.scalePct),
    speedPct: Number(record.speedPct),
    totalPct: isFiniteNumber(record.totalPct)
      ? Number(record.totalPct)
      : Number(record.researchPct) + Number(record.scalePct) + Number(record.speedPct),
  };
}
