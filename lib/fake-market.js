...
function scoreAgainstMarket(candidate, fakeParticipants, sampledRealAttempts) {
  const rankings = buildRankings(buildCompetitionEntries(candidate, fakeParticipants, sampledRealAttempts));
  const candidateRank = rankings.find((entry) => entry.id === candidate.id);

  return scoreAllocation(
    {
      researchPct: candidate.researchPct,
      scalePct: candidate.scalePct,
      totalPct: candidate.totalPct,
    },
    candidateRank
  );
}
